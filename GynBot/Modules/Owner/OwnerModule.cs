namespace GynBot.Modules.Owner
{
    using Discord;
    using Discord.API;
            ...
            [Summary("Value to change the property to"), Remainder] string value)
        {
            switch (prop)
            {
                ...
                case UserProperty.Game:
                    await (Context.Client as DiscordSocketClient).SetGameAsync(value).ConfigureAwait(false);
                    break;

                case UserProperty.Status:
                    var newStatus = Enum.Parse(typeof(UserStatus), value);
                    await
                        (Context.Client as DiscordSocketClient).SetStatusAsync((UserStatus)newStatus).ConfigureAwait(false);
                    break;
