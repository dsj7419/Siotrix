...
namespace Siotrix.Discord.Admin
{
    ...
    public class RequestHelpModule : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task RequestOwnerAsync([Remainder] string report)
        {
            ...
            await ReplyAsync(
                "Notifying a Siotrix Developer...An invite has been sent in case they need to speak with you.");

            var summonMsg =
                $"{devRole.Mention} :eye: {Context.User} from {Context.Guild.Name} submitted a report.\n\nReport: {report.Cap(2000)} \n\n{invite.Url}";
