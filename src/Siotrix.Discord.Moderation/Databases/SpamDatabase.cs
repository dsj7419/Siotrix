...
namespace Siotrix.Discord.Moderation
{
    public class SpamDatabase : DbContext
    {
        ...
        protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
        {
            ...
            var datadir = Path.Combine(AppContext.BaseDirectory, "cache/spam.sqlite.db");
        ...
        public async Task<IEnumerable<SpamMessage>> GetMessagesLike(SpamMessage msg, int distance, int span)
        {
            ...
            var similarMessages =
                channelMessages.Where(x => Levenshtein.GetDistance(x.Content, msg.Content) < distance);
            var chainedMessages =
                similarMessages.Where(x => similarMessages.Any(y => y.CreatedAt >= x.CreatedAt.AddSeconds(-span)));
